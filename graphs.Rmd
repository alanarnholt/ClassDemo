...
ggplot(data = NDF, aes(x = abs, y = hwfat)) + 
  ...
  geom_segment(aes(x = abs, xend = abs, y = hwfat, yend = .fitted), size = 0.15, color = "blue")
```
...
ggplot(data = NDF, aes(x = abs, y = hwfat)) + 
  ...
  geom_segment(aes(x = abs, xend = abs, y = hwfat, yend = .fitted), size = 0.2, color = "gray")
```
...
# Compute R-squared
ndf1 %>%
  summarize(var_y = var(hwfat), var_e = var(.resid)) %>%
